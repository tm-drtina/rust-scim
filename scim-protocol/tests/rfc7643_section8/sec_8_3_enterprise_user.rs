...
use rust_scim::resource::enterprise_user::{EnterpriseUser, Manager, UserResource};
use rust_scim::resource::user::{
...
#[test]
fn test() {
    let actual: UserResource =
        serde_json::from_str(include_str!("./sec_8_3_enterprise_user.json")).unwrap();
    assert_eq!(expected(), actual);
}

fn expected() -> UserResource {
    UserResource {
        schemas: vec!["urn:ietf:params:scim:schemas:core:2.0:User".to_string(), "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User".to_string()],
        common: Common {
